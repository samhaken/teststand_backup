...
int PWMValue = 110;          //initialise PWM duty cycle to 30/255. Fan doesn't run when this value is lower than mid-30s so use 30 as a minimum value.
...
void loop() { //--------------------------------------------MAIN LOOP--------------------------------------------------------------------------------------
    ...
    if (ms_curr - ms_display >= mms) { // "soft interrupt" every mms milliseconds
        ...
        /// set the main nozzle air flow rate
        char setp = "SETP " + ta_setpoint + "\\r\\n";
        Serial2.print(setp);
...
void TA_send_command(char command)
{
